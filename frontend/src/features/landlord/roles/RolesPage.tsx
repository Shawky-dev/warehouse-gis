...
import type { LandlordPermissionOption, LandlordRoleDetails } from "@/features/landlord/types/rbac";
...
const RolesPage = () => {
  const { t } = useI18n();
  const [roles, setRoles] = useState<LandlordRoleDetails[]>([]);
  const [permissions, setPermissions] = useState<LandlordPermissionOption[]>([]);
  ...
  const syncSelectedRoleForm = useCallback((role: LandlordRoleDetails | null) => {
    if (!role) {
      setName("");
      setDescription("");
      setLocked(false);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setPageError(extractRbacErrorMessage(error) ?? t("roles.loadFailed"));
    } finally {
      setIsLoading(false);
    }
  }, [selectedRoleCode, syncSelectedRoleForm, t]);
  ...
  const saveRole = async () => {
    ...
    setFormError(null);
    try {
      await updateLandlordRole(selectedRole.code, {
        name: name.trim(),
      ...
      setFormError(extractRbacErrorMessage(error) ?? t("roles.actionFailed"));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const createRole = async () => {
    ...
    setFormError(null);
    try {
      const created = await createLandlordRole({
            ...
            {!selectedRole ? (
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-72 space-y-2 overflow-y-auto border p-3">
                ...
                {permissions.map((permission) => (
                  <label key={permission.code} className="flex items-start gap-2 text-sm">
                    <input
                      ...
                      onChange={() => toggleCreatePermission(permission.code)}
                    />
